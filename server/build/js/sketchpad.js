...
var RESAMPLE_SIZE = 24,
    RESAMPLE_LEN_PER_SEGMENT = 250;
var lineLength = 0,
    lineMin = 25;
...
function getColorFromType(type) {
  if (type == 'bed') return 'blue';
  if (type == 'wardrobe') return 'green';
  if (type == 'skylight') return 'yellow';
  if (type == 'desk') return 'red';
  return 'error';
}
...
var northAngle = 0,
    northX = CANVAS_WIDTH / 2,
    northY = 5,
    northWidth = 30,
    northHeight = 40;

var northArrow = sketchpadPaper.image("../images/northarrow.png", 0, 0, northWidth, northHeight).attr({ cursor: "move", transform: "R" + northAngle + "T" + northX + "," + northY });
...
$(sketchpad).mouseup(function () {
  if (northArrowClick == true || HOVERING_OBJECT == true) return;
  mousedown = false;
  path.remove();
  ...
  var processed = findPrintedPath(lastpath, startPoint, endPoint, clickedOn, windowMode, shiftDown, RESAMPLE_SIZE);
...
function addStroke(points, windowM) {
  var processed = findPrintedPath(points, points[0], points[points.length - 1], clickedOn, windowM, shiftDown, RESAMPLE_SIZE);
...
function process_line(pts, windowM, clicked) {
  var idname, type;
  if (windowM == true) type = 'window';else if (randomScore(pts, 5) > .075) type = 'scribble';else type = 'linesegment';
...
function scribbleOut() {
  if (Stroke_List[Stroke_List.length - 1].type != 'scribble') return;
...
function drawover() {
  ...
  var j = 3;
  if (Stroke_List.length < 3) j = Stroke_List.length;
  if (j == 1 || curr.type == 'window' || windowMode) return false;

  for (var i = 0; i < j - 1; i++) {
    if (d[i].dist < 15 && withinDiff(d[i].slope, curr.bestFitLine.slope) < 4 && withinDiff(d[i].length, curr.length) < 100 && Stroke_List[d[i].id].removed == false && Stroke_List[d[i].id].type != 'scribble') {
...
function reclassify(letter) {
  ...
  if (closest.dist < 100) {
    ...
    deleteProcess(Stroke_List[Stroke_List.length - 1]);
  } else console.log('hi', closest.dist);
}

//returns choice1 or choice2, based on which one is closer to spt
function findCloser(sPt, choice1, choice2) {
  var d1 = distance(sPt, choice1),
      d2 = distance(sPt, choice2);
  if (d1 > d2) return choice2;else return choice1;
}
...
function findCloser2(sPt, ePt, choice1, choice2) {
  var d1 = distance(sPt, choice1),
      d2 = distance(sPt, choice2),
      d3 = distance(ePt, choice1),
      d4 = distance(ePt, choice2);
  if (d1 > d3) return choice1;else return choice2;
}
...
function findEndPoint(startPt, endPt, strokeId, length) {
  ...
  if (!isFinite(windSlope)) windSlope = 999999;
  if (windSlope == 0) windSlope = .00001;
...
function modelHasWindows() {
  for (var i = 0; i < Stroke_List.length; i++) {
    if (Stroke_List[i].type == 'window') return true;
  }
  return false;
}
...
function findStrokesFromIds(ids) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    if (st != -1) output.push(st);else console.log("ERROR could not find stroke: findStrokes ", 'stroke_' + ids[i]);
  }
  return output;
}
...
function leastSquares(pts) {
  ...
  if (!isFinite(lr['slope'])) lr['slope'] = 999999;
  if (lr['slope'] == 0) lr['slope'] = .00001;
...
function pointsToPath(points) {
  ...
  for (var i = 0; i < points.length; i++) {
    if (i == 0) linepath = linepath + "M" + points[i].x + "," + points[i].y + " ";else linepath = linepath + "L" + points[i].x + "," + points[i].y + " ";
  }
  return linepath;
}
...
function isLine(points, a, b) {
  ...
  if (dist / pathDist > threshold) return true;
  return false;
}
...
function drawLabel(rectangle, text, labelId) {
  var t;
  if ((typeof text === 'undefined' ? 'undefined' : _typeof(text)) === 'object') t = text.name;else t = text;
...
function lineRightAngleCheck(strokes) {
  ...
  var output = [];
  s.sort(function (a, b) {
    return angleAwayFromCenter(center.x, center.y, b.midpoint.x, b.midpoint.y) - angleAwayFromCenter(center.x, center.y, a.midpoint.x, a.midpoint.y);
  });
...
function noWindows(strokes) {
  for (var i = 0; i < strokes.length; i++) {
    if (strokes[i].windows.length != 0) return false;
    if (strokes[i].type == 'window') return false;
  }
  return true;
}
...
function draw_line(pts, idname, type) {
  ...
  if (type == 'window') drawn_line.attr({ "stroke": "#0EBFE9", "stroke-width": 5 });else {
...
function moveStroke(idnum, ft, x, y) {
  ...
  if (currStroke.type == 'window') drawn_line.attr({ "stroke": "#0EBFE9", "stroke-width": 5 });else {
    drawn_line.attr({ "stroke": "#000000", "stroke-width": 3 });
    drawn_line.mouseout(hoverout);
    drawn_line.mouseover(hovering);
    drawn_line.mousedown(pathMouseDown);
...
function onContextMenuShow(target, pos) {
  ...
  switch (s.type) {
    case 'rect':
      ...
      $('#menuRect').find('a[name="size"]').removeAttr('checked').parent().removeClass('checked').end().end().find('a[name="size"][w="' + w + '"]').attr('checked', 'checked').parent().addClass('checked');
      break;
  }

  $('#menuRect, #menuCircle').find('a[name="style"]').removeAttr('checked').parent().removeClass('checked').end().end().find('a[name="style"][val="' + s.class + '"]').attr('checked', 'checked').parent().addClass('checked');
}
...
function rectDistance(centerx, centery, width, height, px, py) {
  ...
  //it's inside
  if (dx == 0 && dy == 0) {
    return Math.min(Math.abs(py - centery - height / 2), Math.abs(py - centery + height / 2), Math.abs(px - centerx - width / 2), Math.abs(px - centerx + width / 2));
  }
...
function recursiveScoring(points, rect, prevScore, inc) {
  ...
  if (inc > 10) {
    if (currScore > prevScore) return { score: prevScore, rect: rect };else return { score: currScore, rect: rect };
  }

  if (currScore > prevScore && currScore < prevScore) return { score: prevScore, rect: rect };else {
...
function iterativeScoring(points, rect, prevScore, inc) {
  var aInc = incrementCalc(inc, 'angle'),
      whInc = incrementCalc(inc, 'wh'),
      xyInc = incrementCalc(inc, 'xy');
  //score the current rect
  var output = [],
      results = [];
  var rects = [{ cx: rect.cx, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle + aInc }, { cx: rect.cx, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle - aInc }, { cx: rect.cx, cy: rect.cy, w: rect.w + whInc, h: rect.h, angle: rect.angle }, { cx: rect.cx, cy: rect.cy, w: rect.w, h: rect.h + whInc, angle: rect.angle }, { cx: rect.cx, cy: rect.cy, w: rect.w - whInc, h: rect.h, angle: rect.angle }, { cx: rect.cx, cy: rect.cy, w: rect.w, h: rect.h - whInc, angle: rect.angle }, { cx: rect.cx + xyInc, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle }, { cx: rect.cx, cy: rect.cy + xyInc, w: rect.w, h: rect.h, angle: rect.angle }, { cx: rect.cx - xyInc, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle }, { cx: rect.cx, cy: rect.cy - xyInc, w: rect.w, h: rect.h, angle: rect.angle }];

  var actions = ['+angle', '-angle', '+width', '+height', '-width', '-height', '+xaxis', '+yaxis', '-xaxis', '-yaxis'];
  for (var i = 0; i < rects.length; i++) {
...
function iterativeScoringFixedSize(points, rect, prevScore, inc) {
  ...
  var output = [],
      results = [];
  var rects = [{ cx: rect.cx, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle + aInc }, { cx: rect.cx, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle - aInc }, { cx: rect.cx + xyInc, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle }, { cx: rect.cx, cy: rect.cy + xyInc, w: rect.w, h: rect.h, angle: rect.angle }, { cx: rect.cx - xyInc, cy: rect.cy, w: rect.w, h: rect.h, angle: rect.angle }, { cx: rect.cx, cy: rect.cy - xyInc, w: rect.w, h: rect.h, angle: rect.angle }];
...
function exportStrokes(id, name, owner, rects, north, scale) {
  ...
  for (var i = 0; i < Stroke_List.length; i++) {
    if (Stroke_List[i].removed == false) {
      ...
      if (curr.type == 'window') {} else if (x == -1) {
        if (withinPercent(curr.length, distance(curr.points[0], curr.points[curr.points.length - 1])) < .001) pts = [curr.points[0], curr.points[curr.points.length - 1]];else {
          pts = curr.points;
        }
        var w = [];
...
function aContainsB(arr, b) {
  for (var i = 0; i < arr.length; i++) {
    if (arraysEqual(arr[i].strokes, b.strokes)) return true;
  }
  return false;
}
