...
function isWallValid(table, uL, uR, lR, lL) {
  return onTable(table, uL[0], uL[1]) && onTable(table, uR[0], uR[1]) && onTable(table, lR[0], lR[1]) && onTable(table, lL[0], lL[1]);
} //isvld
...
function inch2meter(num) {
  return num / 39.3701;
}
function meter2inches(num) {
  return num * 39.3701;
}
...
function generateTransData(ftWallArray, ftWindowArray, ftNorth, table, INITAL_VIEW_SIZE_W, INITAL_VIEW_SIZE_H, time_start) {
  ...
  result_str += "viewport " + INITAL_VIEW_SIZE_W.toString() + " " + INITAL_VIEW_SIZE_H.toString() + "\n";
  ...
  for (var i = 0; i < ftWallArray.length; i++) {
    ...
    if (wall.isCurved) {
      result_str += "wall" + " " + id + " " + trans_x + " " + trans_y + " " + scale_x + " " + scale_y + " " + rot + " curved\n";
    } else {
      result_str += "wall" + " " + id + " " + trans_x + " " + trans_y + " " + scale_x + " " + scale_y + " " + rot + " not_curved\n";
    }
  }
  ...
  for (var i = 0; i < ftWindowArray.length; i++) {
    ...
    result_str += "win" + " " + id + " " + trans_x + " " + trans_y + " " + scale_x + " " + scale_y + " " + rot + " " + pair + "\n";
  }
    ...
    if (isWallValid(table, uL, uR, lR, lL) && wall.subject.attr('opacity') > 0) {
      ...
      if (wall.idPairArray.length != 0) {
        ...
        for (var j = 0; j < wall.idPairArray.length; j++) {
          ...
          x = curWin.attrs.x + curWin.attrs.translate.x, y = curWin.attrs.y + curWin.attrs.translate.y, w = curWin.attrs.size.x * curWin.attrs.scale.x, h = curWin.attrs.size.y, a = curWin.attrs.rotate;
...
function onHandleSide(ftWall, point) {
  ...
  if (center_x < handle_x) {
    if (center_x <= point.x && point.x < handle_x) {
      return true;
    } else {
      return false;
    }
  } else if (center_x > handle_x) {
    if (handle_x <= point.x && point.x < center_x) {
      return true;
    } else {
      return false;
    }
    ...
    if (center_y < handle_y) {
      if (center_y <= point.y && point.y < handle_y) {
        return true;
      } else {
        return false;
      }
    } else if (center_y > handle_y) {
      if (handle_y <= point.y && point.y < center_y) {
        return true;
      } else {
        return false;
      }
...
function xScale(x) {
  return x * (width / WIDTH_CANVAS);
};
function yScale(y) {
  return y * (height / HEIGHT_CANVAS);
};
